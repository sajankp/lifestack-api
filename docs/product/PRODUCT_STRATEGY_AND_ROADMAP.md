...
merged) landed the tool-wiring fix, disconnect instrumentation, and eval harness, but the
recalibrated run measured **84.2% (16/19)**, below the 90% bar — Stages B (transport resilience)
and C (multi-item capture) stay gated per the spec's own rule, and the "experimental" label stays
on voice/capture. §4 "Immediate Focus" numbered list and the implemented-waves table below updated
to match; this pass also caught the roadmap doc still saying "(Draft)" for 075/077 despite both
having shipped earlier the same day — drift now closed.
...
2. ~~**Currency display polish**~~ — **done 2026-07-12** (spec-075, api#155/web#113): [spec-075](../specs/spec-075-currency-display-consistency.md). Display profile, as-of historical FX replay with rate provenance.
3. **Weekly summaries enhancements** — [spec-076](../specs/spec-076-weekly-summaries-enhancements.md) (Approved, deliberately skipped this pass): per-workspace cadence, versioned regeneration, dividend/net-worth/return sections.
4. ~~**Custom financial KPIs**~~ — **done 2026-07-12** (spec-077, api#156/web#114): [spec-077](../specs/spec-077-custom-financial-kpis.md). Predefined metric types, guardrails-cadence evaluation; expression language fenced off.
...
7. **PostHog observability + Resend email channel** — [spec-081](../specs/spec-081-posthog-analytics-and-resend-email.md) (Approved 2026-07-14, not started): frontend analytics (explicit events only) + frontend/backend exception capture via PostHog free tier; email delivery for the dormant `channel_email` preference (spec-052) via Resend free tier. Fully inert without env keys; zero recurring cost.
8. **OpenTelemetry traces/logs → PostHog** — [spec-082](../specs/spec-082-opentelemetry-traces-logs-to-posthog.md) (Approved 2026-07-14, gated on spec-081 shipping first): vendor-neutral OTel instrumentation exporting to PostHog's OTLP endpoint, replacing the earlier self-hosted Grafana plan; the redaction processor is the core design problem; metrics explicitly out of scope.
