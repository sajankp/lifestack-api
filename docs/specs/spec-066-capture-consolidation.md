...
- **Lazy connection (owner decision):** opening the panel does NOT open the WebSocket. The connection is established as soon as the user focuses the text input, starts typing (on first keystroke), or activates the mic, rather than waiting for the actual submission. This minimizes latency while still avoiding auto-connecting on page load. It is torn down per the existing session limits. The launcher never auto-connects.
