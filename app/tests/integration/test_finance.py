...
# ---------------------------------------------------------------------------
# Historical data ingestion (spec-072): user FX rates + net-worth backfill
# ---------------------------------------------------------------------------


async def _workspace_id_for_username(username: str) -> int:
    async with postgres.async_session_maker() as session:
        user = (
            (await session.execute(select(User).where(User.username == username))).scalars().one()
        )
        membership = (
            (
                await session.execute(
...
@pytest.mark.asyncio
async def test_fx_history_import_idempotent_and_amount_update(client: AsyncClient):
    await _register_and_login(
        client,
        email="fx-history-import@example.com",
        username="fx-history-import",
        password="TestPass123!",
    )
    row = {
        "base_currency_code": "USD",
    ...
    res1 = await client.post("/v1/finance/fx/history/import", json={"rows": [row]})
    assert res1.status_code == 200, res1.text
    assert res1.json() == {"imported": 1, "skipped": 0, "rejected": []}

    # Exact re-upload is a no-op (INV-4 idempotency).
    res2 = await client.post("/v1/finance/fx/history/import", json={"rows": [row]})
    assert res2.json() == {"imported": 0, "skipped": 1, "rejected": []}

    # A corrected rate at the same key updates in place (upsert).
    corrected = {**row, "rate": "72.0000000000"}
    res3 = await client.post("/v1/finance/fx/history/import", json={"rows": [corrected]})
    assert res3.json() == {"imported": 1, "skipped": 0, "rejected": []}

    list_res = await client.get("/v1/finance/fx/history")
    items = list_res.json()["items"]
    assert len(items) == 1
    assert Decimal(items[0]["rate"]) == Decimal("72.0000000000")


@pytest.mark.asyncio
async def test_fx_history_import_rejects_future_date(client: AsyncClient):
    await _register_and_login(
        client,
        email="fx-history-future@example.com",
        username="fx-history-future",
        password="TestPass123!",
    )
    future_date = (datetime.now(UTC).date() + timedelta(days=10)).isoformat()
    res = await client.post(
        "/v1/finance/fx/history/import",
        json={
            "rows": [
                {
                    "base_currency_code": "USD",
    ...
    assert "future" in body["rejected"][0]["reason"]


@pytest.mark.asyncio
async def test_fx_history_delete_row(client: AsyncClient):
    await _register_and_login(
        client,
        email="fx-history-delete@example.com",
        username="fx-history-delete",
        password="TestPass123!",
    )
    await client.post(
        "/v1/finance/fx/history/import",
    ...
    items = (await client.get("/v1/finance/fx/history")).json()["items"]
    row_id = items[0]["id"]

    delete_res = await client.delete(f"/v1/finance/fx/history/{row_id}")
    assert delete_res.status_code == 204

    items_after = (await client.get("/v1/finance/fx/history")).json()["items"]
    assert items_after == []


@pytest.mark.asyncio
async def test_historical_rate_resolution_system_beats_user(client: AsyncClient):
    """INV-3: system rate always wins when both exist for the same date;
    user rate is used only as a fallback, and never leaks into live/current
    rate resolution (get_latest_pair / /finance/fx-rates)."""
    await _register_and_login(
        client, email="fx-precedence@example.com", username="fx-precedence", password="TestPass123!"
    )
    workspace_id = await _workspace_id_for_username("fx-precedence")

    as_of_date = date(2021, 6, 15)
    as_of_dt = datetime(2021, 6, 15, tzinfo=UTC)
    ...
    async with postgres.async_session_maker() as session:
        ...
        service = FxRateService(fx_repo, currency_repo)

        # No system rate yet -> falls back to the user-provided rate.
        result = await service.resolve_historical_rate(workspace_id, "USD", "INR", as_of_dt)
        assert result == (Decimal("70.0000000000"), "user_provided")

        # A system rate for the same pair/date now takes precedence.
        await fx_repo.upsert_rate(
            base_currency_code="USD",
            quote_currency_code="INR",
            rate=Decimal("75.0000000000"),
            as_of=as_of_dt,
            fetched_at=as_of_dt,
            source="test-system-feed",
        )
        await session.commit()
    ...
    async with postgres.async_session_maker() as session:
        ...
        service = FxRateService(fx_repo, currency_repo)
        result = await service.resolve_historical_rate(workspace_id, "USD", "INR", as_of_dt)
        assert result == (Decimal("75.0000000000"), "system")

        # get_latest_pair (live/current path) must never see the user row --
        # it should resolve straight to the system rate, not error or blend.
        live = await service.get_latest_pair("USD", "INR", as_of=as_of_dt)
        assert live is not None
        assert Decimal(str(live.rate)) == Decimal("75.0000000000")


@pytest.mark.asyncio
async def test_net_worth_backfill_import_before_boundary_accepted(client: AsyncClient):
    await _register_and_login(
        client,
        email="nw-backfill-ok@example.com",
        username="nw-backfill-ok",
        password="TestPass123!",
    )
    past_date = (datetime.now(UTC).date() - timedelta(days=400)).isoformat()
    res = await client.post(
        "/v1/finance/net-worth/history/import",
        json={
            "rows": [
                {
                    "date": past_date,
    ...
    assert res.json() == {"imported": 1, "skipped": 0, "rejected": []}

    points = (await client.get("/v1/finance/net-worth/history/user-points")).json()["items"]
    assert len(points) == 1
    assert points[0]["total_net_worth"] == "10000.00"
    assert points[0]["holdings_value"] is None

    history = await client.get(
        "/v1/finance/net-worth/history",
        params={"from_date": past_date, "to_date": past_date},
    )
    row = history.json()[0]
    assert row["source"] == "user_provided"
    assert row["holdings_value"] is None


@pytest.mark.asyncio
async def test_net_worth_backfill_rejects_on_or_after_boundary(client: AsyncClient):
    """INV-2: with no live snapshot yet, the boundary is today -- a point
    dated today or later is rejected (date_not_backfill), not silently
    accepted, so the daily job can never collide with it."""
    await _register_and_login(
        client,
        email="nw-backfill-boundary@example.com",
        username="nw-backfill-boundary",
        password="TestPass123!",
    )
    today = datetime.now(UTC).date().isoformat()
    res = await client.post(
        "/v1/finance/net-worth/history/import",
        json={"rows": [{"date": today, "total_net_worth": "5000.00", "reporting_currency": "USD"}]},
    )
    body = res.json()
    assert body["imported"] == 0
    assert len(body["rejected"]) == 1
    assert "date_not_backfill" in body["rejected"][0]["reason"]


@pytest.mark.asyncio
async def test_net_worth_backfill_component_sum_validation(client: AsyncClient):
    await _register_and_login(
        client,
        email="nw-backfill-sum@example.com",
        username="nw-backfill-sum",
        password="TestPass123!",
    )
    past_date = (datetime.now(UTC).date() - timedelta(days=400)).isoformat()
    ...
    mismatched = {
        ...
        "holdings_value": "500.00",
        "investing_cash": "200.00",
        "spending_cash": "200.00",  # sums to 900, not 1000
        "reporting_currency": "USD",
    }
    res = await client.post("/v1/finance/net-worth/history/import", json={"rows": [mismatched]})
    body = res.json()
    assert body["imported"] == 0
    assert "sum" in body["rejected"][0]["reason"]
    ...
    res2 = await client.post("/v1/finance/net-worth/history/import", json={"rows": [partial]})
    body2 = res2.json()
    assert body2["imported"] == 0
    assert "all given or all omitted" in body2["rejected"][0]["reason"]


@pytest.mark.asyncio
async def test_net_worth_backfill_delete_reopens_gap(client: AsyncClient):
    await _register_and_login(
        client,
        email="nw-backfill-delete@example.com",
        username="nw-backfill-delete",
        password="TestPass123!",
    )
    past_date = (datetime.now(UTC).date() - timedelta(days=400)).isoformat()
    await client.post(
        ...
        json={
            "rows": [
                {"date": past_date, "total_net_worth": "10000.00", "reporting_currency": "USD"}
            ]
        },
    )
    points = (await client.get("/v1/finance/net-worth/history/user-points")).json()["items"]
    ...
    delete_res = await client.delete(f"/v1/finance/net-worth/history/user-points/{point_id}")
    assert delete_res.status_code == 204

    points_after = (await client.get("/v1/finance/net-worth/history/user-points")).json()["items"]
    assert points_after == []
