...
| Corporate actions (stock splits) | Per-account corporate-action support: enter a split ratio and ex-date for a symbol and auto-adjust all affected lots/orders (quantity up, price down), instead of hand-editing historical orders. | Holding quantity/avg_cost are replayed from orders, so an un-applied split (e.g. NVDA 10:1, GOOGL 20:1 in imported IND Money data) silently understates share count and distorts FIFO cost basis and realized P/L. A first-class split action is the correct long-term fix; manual order edits are the interim workaround. |
| Brokerage in cost basis | Capitalize buy-side brokerage/fees into FIFO lot cost (and net sell-side fees from proceeds) so `book_value`/`avg_cost` match broker apps and tax cost-of-acquisition convention. Optional separate fees column for transparency. | Currently fees are stored per order but excluded from cost basis, so invested is under-reported and unrealized gain over-reported by the fee total (~$44 across imported IND Money orders). Needs a spec; also note imported GROWW orders carry $0 fees, so they stay gross-only until backfilled. |
