...
| --- | --- | --- | --- |
| Milestone 1 - Demo Safety Baseline | Partial | Demo reset is feature-flagged, role-gated for owner/admin, available through backend and Master Config UI, and covered by integration tests. | Verify active-workspace targeting, audit events, confirmation phrase behavior, and README/spec seed-data alignment. |
| Milestone 2 - Workspace and Session Correctness | Partial | Workspace selection updates the active workspace token claim, persists refresh-token hash rotation, and is covered for follow-up workspace-aware actions plus select-then-refresh. | Central session rotation helper, explicit frontend active-workspace model, and destructive-flow workspace visibility. |
...
| Milestone 4 - Documentation Reconciliation | Partial | This roadmap now identifies current surfaces, future-track non-goals, and the demo journey. | README, ERD, historical roadmap statuses, and demo reset seed docs still need final reconciliation. |
| Milestone 5 - Maintainability Cleanup | Open | Background lifecycle and DB hardening work from Gate 0 reduced some operational risk. | Extract demo reset service, split large dependency/frontend modules, and replace container-shell E2E helpers. |
