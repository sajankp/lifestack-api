...
def create_mcp_server() -> FastMCP:
    ...
    @mcp.tool
    async def create_todo(workspace_id: int, title: str, description: str | None = None,
        ...
        async with postgres.async_session_maker() as session, session.begin():
            user_id = await authorize_workspace(session, workspace_id, required_scope="mcp:write", tool="create_todo")
            due = datetime.fromisoformat(due_date) if due_date else None
            todo = await TodoService(TodoRepository(session)).create_todo(
                workspace_id, user_id, TodoCreate(title=title, description=description, due_date=due)
            )
            return TodoResponse.model_validate(todo).model_dump()

    @mcp.tool
    async def list_todos(workspace_id: int, completed: bool | None = None,
                         limit: int = 20, offset: int = 0) -> dict[str, Any]:
        """List todos for an authorized workspace."""
        validate_page(limit, offset)
        async with postgres.async_session_maker() as session:
            await authorize_workspace(session, workspace_id, required_scope="mcp:read", tool="list_todos")
            todos, total = await TodoService(TodoRepository(session)).list_todos(workspace_id, completed, limit, offset)
            return {"items": [TodoResponse.model_validate(t).model_dump() for t in todos], "total": total, "limit": limit, "offset": offset}

    @mcp.tool
    async def get_todo_summary(workspace_id: int) -> dict[str, Any]:
        """Get todo counts and due items for an authorized workspace."""
        async with postgres.async_session_maker() as session:
            await authorize_workspace(session, workspace_id, required_scope="mcp:read", tool="get_todo_summary")
            ...
            return {"workspace_id": workspace_id, "pending": pending, "completed": completed,
                    "overdue_count": len(overdue), "overdue_items": [TodoResponse.model_validate(t).model_dump() for t in overdue],
    ...
    async def get_net_worth(workspace_id: int) -> dict[str, Any]:
        ...
        async with postgres.async_session_maker() as session:
            await authorize_workspace(session, workspace_id, required_scope="mcp:read", tool="get_net_worth")
    ...
    async def get_spending_budgets(workspace_id: int) -> dict[str, Any]:
        ...
        async with postgres.async_session_maker() as session:
            await authorize_workspace(session, workspace_id, required_scope="mcp:read", tool="get_spending_budgets")
            service = BudgetService(BudgetRepository(session), CategoryRepository(session), CategoryGroupRepository(session))
            budgets, total = await service.list_budgets_with_details(workspace_id, limit=100)
            return {"workspace_id": workspace_id, "budgets": [b.model_dump() for b in budgets], "total": total}
