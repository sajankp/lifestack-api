...
**Created:** 2026-07-10
**Status:** Implemented (api backend) — owner-approved and built 2026-07-11. New `GET /investing/performance/returns`: `app/investing/xirr.py` (pure Newton→bisection solver, bounded, 9 hand-checked unit tests) + `ReturnMetricsService` (`app/investing/return_metrics_service.py`) computing overall/by_account/by_currency scopes, each split into open/closed with realized/unrealized, `annualization_reliable` (INV-7), and overall-only `max_drawdown` from live `net_worth_snapshots` (investing value = holdings + investing cash). Cross-currency aggregate converts each flow at its own historical rate via spec-072's `FxRateService.resolve_historical_rate`, returning `valuation_status='conversion_required'` when a rate is missing. 7 integration tests (open/closed partition exhaustiveness INV-5, dividend-as-income INV-6, sub-year guard INV-7, terminal-value inclusion). **Web UI implemented** (lifestack-web, same commit series): `ReturnMetricsPanel` on the Investing summary page — overall XIRR/annualized headline (with the INV-7 simple-return-plus-holding-period fallback for sub-year positions), a max-drawdown badge, a Current-holdings/Exited-positions segmented toggle with realized/unrealized, and a by-account grid when multiple accounts exist. Shows an explanatory `conversion_required` state pointing at the spec-072 historical-FX uploader instead of a blended wrong number. Rev. 2 resolved the multi-currency and drawdown open questions per owner decision (historical-FX aggregate; include drawdown) and added the dependency on spec-072. Rev. 3 pinned drawdown to overall-only (verified: `net_worth_snapshots` is workspace-level — no per-account dimension exists) and defined its input as investing value; added the sub-year annualization guard (INV-7); included income flows in closed-position XIRR; pinned the day-count convention.
