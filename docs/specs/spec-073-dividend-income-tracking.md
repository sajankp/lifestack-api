...
- **INV-6 — Deletion reverses cash via a compensating snapshot row.** Dividends never touch holdings or FIFO lots, so there is **no order-style replay**. Create appends a snapshot credit (`prev + net`); delete appends a compensating debit (`prev − net`) referencing the same `trigger_ref`; edit of `net_amount` appends the delta — exactly the transfer-deletion pattern, per the snapshot append-only model.
...
Integration:
- **Cash**: create appends an `investing_cash_balances` credit row with `trigger_type='dividend'`, `trigger_ref=<dividend public_id>` (mirrors transfers/orders); delete/edit append compensating rows (INV-6). Resolution always via `get_by_trigger_ref_and_account`, never the unscoped lookup.
...
- INV-6: delete appends a compensating snapshot debit (no replay); edit appends the delta; snapshot series stays append-only and reconciliation discrepancy stays 0 throughout.
