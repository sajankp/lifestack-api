...
**Created:** 2026-07-10
**Status:** Draft (rev. 2, 2026-07-10) — awaiting approval. No code until approved. Rev. 2 resolves the FX-store question (extend `fx_rates`, workspace-scoped user rows), fixes the net-worth schema contradiction (component columns are currently NOT NULL — migration makes them nullable), closes the daily-job/user-row collision gap (backfill dates must precede live history), and documents currency behavior for user points.
